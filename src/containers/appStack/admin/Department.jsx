...
const Department = () => {
  ...
  const getAllProducts = () => {
    setIsLoading(true);
    getAllDepartments(currentPage, adminRole?.token)
      .then((res) => {
        if (res.status === 200) {
  ...
  const showAlert=()=>{
    swal({
      title: "Are you sure?",
      text: "Once deleted, you will not be able to recover these categories and it will effect the products of these categoires!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    })
  ...
  const handleDeleteProducts = () => {
    ...
    setIsLoading(true);
    deleteDepartment(body, adminRole?.token)
      .then((res) => {
        if (res?.status == 200) {
          getAllProducts();
          setIsLoading(false);
                      ...
                      {getProductData?.length >  0 ? (
                        ...
                        <TableCell colSpan={7} align="center">
                          <div className="no-product text-center d-flex align-items-center justify-content-center flex-column mt-5">
                            ...
                            <h4 className='text-muted mt-2'>No Department Found</h4>
                          </div>
                        </TableCell>
                      </TableRow>
                      )}
